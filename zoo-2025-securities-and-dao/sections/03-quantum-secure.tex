...
Corona Ring-LWE & Ring Learning-With-Errors (post-quantum lattice).
   See \Lux{} \textsc{lp-073}~\cite{lp-073-corona,ring-lwe}.
   & Post-quantum threshold signature; co-signed with \BLS{} on every
   block. Falls back to majority Corona-only certificate if \BLS{}
   is suspect. \\
...
\[
\sigma \;=\; \big( \sigma_{\BLS{}},\; \sigma_{\text{Corona}},\; \pi_{\MLDSA{}} \big)
\]

\noindent where $\sigma_{\BLS{}}$ is the aggregate $\BLS{}_{12\text{-}381}$
signature from the validator quorum, $\sigma_{\text{Corona}}$ is the
...
\item Forging $\sigma_{\text{Corona}}$ requires solving Ring-LWE for
...
\item \emph{Post-Q-day}: \BLS{} compromise alone does not break
  finality. The Corona + $\MLDSA{}$ pair is sufficient for
...
\item \emph{No silent break}: an attacker who silently obtains a
  ...
  Corona + $\MLDSA{}$ proofs remain on-chain in every prior block.
\end{itemize}

\subsection{Settlement guarantees for Zoo securities workloads}
...
\item \textbf{Custody is post-quantum-safe.} M-Chain (\Lux{}
  ...
  using Corona \textsc{pq} threshold signatures.
...
\item \Lux{} \textsc{lp-073} --- Corona Ring-LWE threshold~\cite{lp-073-corona}.
