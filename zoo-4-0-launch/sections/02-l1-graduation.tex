...
4.0 \Zoo{} graduates to a sovereign L1 with its own validator set, its own
...
\toprule
\textbf{Concern} & \textbf{Zoo 2.x (L2)} & \textbf{Zoo 4.0 (L1)} \\
\midrule
...
\paragraph{Native confidential compute.} \Zoo{} 4.0 exposes
...
\begin{itemize}[leftmargin=1.2em]
  \item \textbf{Q-Chain} runs the \Ringtail{} threshold ceremonies that
...
gas asset of the block. Quasar 4.0 triple-consensus
(\textsc{lp-020}: BLS + Ringtail + ML-DSA) means every signed block carries
...
\Ringtail{} layer must independently authorize. Slashing is enforced via
