...
\medskip\noindent\textbf{Post-quantum by construction.} The reference consensus---the \emph{Quasar} round protocol---authenticates validator identity, handshakes, and block signatures with NIST post-quantum primitives: ML-DSA (FIPS~204) and SLH-DSA (FIPS~205) signatures and ML-KEM (FIPS~203) key exchange---lattice- and hash-based, with no elliptic curve for a quantum computer to break. The proof path is post-quantum too: \emph{P3Q}, a pairing-free STARK/FRI system, lets the zkML evidence of Table~\ref{tab:evidence} be a post-quantum proof. So a signed judgment, a \PoT{} receipt, or an attestation is authenticated under post-quantum signatures, and the Cognition precompile carries the same guarantees to any EVM that adopts it. A Thinking Chain is post-quantum exactly where it matters: the authentication of the artifacts consensus consumes.
