...
so the total ever minted by subsidy is the geometric sum $\sum_{t\ge0}\sigma(t)=2\sigma_0 T_h=S_{\max}=10^9$ \textsc{ai}. The entire one billion is mined into existence by useful cognition: each block's subsidy is split among that block's cognitive nodes in proportion to their accepted, settled work, so there is no premine and no coin without a thought behind it. The running supply $M(t)=\sum_{s\le t}\sigma(s)$ rises monotonically to, but never exceeds, the $10^9$ cap (Appendix~\ref{app:proofs}, Theorem~\ref{thm:issue}).
...
\medskip\noindent\textbf{Routing.} Demand flows to nodes by cognitive weight $W_i=B_i^{\alpha}R_i^{\beta}D_i^{\gamma}$ (bond $B$, reputation $R$, demand $D$; $\alpha<1$ blunts stake oligarchy), sampled with probability $W_i/\sum_j W_j$ under a multiplicative-weights update, so persistent usefulness attracts exponentially more flow (Theorem~\ref{thm:flow}).
...
\label{thm:pot}
Let the evidence type $e$ of receipt $r$ have a verifier whose trust assumption fails with probability at most $\eta_e$ (signature forgery for signatures/quorum, enclave compromise for attestation, proof unsoundness for zkML, and $\eta_e=0$ for deterministic recomputation). Then $\Pr[\,V_{\PoT}(S,r)=1 \text{ yet } c_{\mathrm{out}} \text{ is not a correct evaluation of } m \text{ on } c_{\mathrm{in}}\,]\le \eta_e$. A policy requiring $k$ independent evidence types attains $\prod_{j=1}^k \eta_{e_j}$.
\end{theorem}
\begin{proof}
...
\begin{theorem}[Quorum capture bound]
\label{thm:quorum}
A quorum task samples a committee of $n$ nodes from a pool of adversarial weight fraction $p$, certifying an output at threshold $\theta n$. If $p<\theta$, a false certification occurs with probability at most $\exp\!\big(-2n(\theta-p)^2\big)$.
\end{theorem}
\begin{proof}
Let $X$ be the adversarial count (weight) in the sample, $\mathbb{E}X=pn$. A false output requires $X\ge\theta n=(p+\delta)n$ with $\delta=\theta-p>0$. Hoeffding's upper tail gives $\Pr[X\ge(p+\delta)n]\le e^{-2n\delta^2}$.
\end{proof}

\noindent Thus quorum's $\eta_e$ in Theorem~\ref{thm:pot} is itself exponentially small in committee size for any honesty margin $\theta-p>0$.
...
\label{lem:dissent}
Let the penalty map satisfy $\pi(a)>0\Rightarrow a\in\mathcal{O}$, where $\mathcal{O}$ is the set of objectively, deterministically checkable faults (double-sign, invalid receipt, malformed artifact). Then no well-formed good-faith judgment is ever penalized, and reporting one's true posterior weakly dominates: it incurs no $\pi$ and maximizes expected reputation, whose bounded EMA rewards agreement with the canonical outcome and never debits for honest divergence.
\end{lemma}
\begin{proof}
A good-faith judgment is finite, well-formed, and signed, hence in $\mathcal{S}=\neg\mathcal{O}$, where $\pi\equiv0$. Misreporting cannot reduce any $\pi$ (already zero on $\mathcal S$) and, the EMA being a proper bounded reward for matching the eventual canonical outcome, lowers expected reputation. Hence truthful reporting weakly dominates and dissent is never slashed.
\end{proof}
