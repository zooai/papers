...
\medskip\noindent\textbf{Routing.} Demand flows to nodes by cognitive weight $W_i=B_i^{\alpha}R_i^{\beta}D_i^{\gamma}$ (bond $B$, reputation $R$, demand $D$), sampled with probability $W_i/\sum_j W_j$ under a multiplicative-weights update, so persistent usefulness attracts exponentially more flow (Theorem~\ref{thm:flow}). \emph{Sybil caveat.} A concave bond exponent ($\alpha<1$) does \emph{not} blunt oligarchy when identities are cheap---it does the opposite: splitting a bond $B$ across $m$ identities yields combined weight $m^{1-\alpha}B^{\alpha}>B^{\alpha}$, so concavity \emph{rewards} fragmentation. Sybil-resistance must therefore come from a non-splittable scarce factor---a per-identity registration cost and stake floor that make splitting net-negative, reputation that cannot be parallel-farmed from one backend, or a per-identity cap on committee weight---\emph{not} from the exponent. The companion \emph{Zoo DAO} paper specifies the identity-cost and weight-cap parameters; here we only flag that the routing weight alone is not Sybil-proof.
