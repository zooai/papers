...
\textbf{Sybil-resistance is not free, and reputation is not a truth oracle.} Two honest limits the core paper defers here. First, routing/selection weight must resist identity-splitting: a concave bond exponent alone \emph{rewards} fragmentation (splitting $B$ across $m$ identities gives $m^{1-\alpha}B^\alpha>B^\alpha$), so the DAO sets a non-splittable cost---a per-identity registration bond and stake floor that make splitting net-negative---and a \emph{per-identity cap} on committee weight, so no single mind (however it is sharded) carries more than $w_{\max}$ of a quorum. Safety-critical quorums certify by \emph{equal seats} (one node, one vote, selection independent of weight), the regime where the capture bound is exponentially small. Second, the reputation EMA rewards agreement with the \emph{canonical} outcome, which is a coordination signal, not ground truth---so for \emph{subjective} tasks it does not by itself make honest reporting dominant (a node could chase what it predicts the quorum will settle). Where an objective outcome is later revealed, scoring is done against \emph{that} (a proper scoring rule), restoring truthful reporting as optimal; where it is not, high-stakes subjective decisions use a bonded dispute game (a challenger stakes against a claim; ground-truth or a higher quorum resolves it) rather than the bare EMA. Reputation curates ordinary judgment cheaply; stake-backed disputes secure the decisions that move money.
